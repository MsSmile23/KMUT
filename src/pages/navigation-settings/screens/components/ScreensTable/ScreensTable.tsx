...
const ScreensTable: FC = () => {
        ...
        actions: (
            <div style={{ display: 'flex', gap: 15 }}>
                ...
                <ButtonEditRow 
                    ...
                    onClick={() => {
                        navigate(getURL(
                            `${ROUTES.SCREENS}/${ROUTES_COMMON.UPDATE}/${i}`,
                            'manager'
                        ), 
    ...
    const updatePageConstructors = async (newArray: any[]) => {
        const checkConfig = findConfig(CONFIG_MNEMOS.FRONT_SCREENS)
        ...
        const resp = checkConfig
            ? await SERVICES_CONFIG.Models.patchConfigByMnemo(CONFIG_MNEMOS.FRONT_SCREENS, {
                mnemo: CONFIG_MNEMOS.FRONT_SCREENS,
        ...
        if (resp.success) {
            if (resp.data) {
                ...
                message.success('Экраны успешно загружены')
            }
        } else {
    ...
    const downLoadButtonHandler = () => {
        ...
        downloadBlob(JSON.stringify(content), 'screens.json', 'json')
    }
                        ...
                        buttons={{ 
                            left: [
                                <ButtonAdd
                                    ...
                                    onClick={() => {
                                        navigate(getURL(
                                            `${ROUTES.SCREENS}/${ROUTES_COMMON.CREATE}`,
                                            'manager'
                                        ))
                                        // navigate(`/${ROUTES.PAGE_MENU_CONSTRUCTOR}/${ROUTES_COMMON.CREATE}`)
                                    }}
                                />,
                                <ECTooltip key="button-download" title="Экспортировать экраны">
                                    <Buttons.BaseButton
                                        size="large"
                                ...
                                <Upload
                                    ...
                                    beforeUpload={async (file) => {
                                        ...
                                        if (parseFile.mnemo == CONFIG_MNEMOS.FRONT_SCREENS) {
                                            updatePageConstructors(parseFile?.value ?? [])
                                        }
                                        else {
                                            message.error('Ошибка при импорте экранов. Неверно указана мнемоника')
                                        }
                                ...
                                >
                                    <ECTooltip key="button-download" title="Загрузить экраны">
                                        <Buttons.BaseButton
                                            size="large"
                                            icon={<UploadOutlined />}
                                            shape="circle"
