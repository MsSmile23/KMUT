...
import { EditedColumn } from '@shared/ui/tables/ECTable2/EditTable/EditedColumn'
...
import { MarkoTestEditColumn } from './MarkoTestEditColumn'
...
export const MarkoTestCopyEditedCom: FC<IEditedColumnModalProps> = ({
    columns = [],
    tableId,
    onSave,
    onReset,
    ...
    useEffect(() => {
        ...
        // console.log('Данные, которые мы получаем в функции обновления, что отслеживает модалку', sortableColumns.data)
    ...
    const updateSortableColumns = (key: string, fields: Record<string, any>) => {
        const updatedColumns = sortableColumns.data.map((col) => {
            if (col.key === key) {
                return {
                    ...col,
                    hide: fields.hide === undefined ? col.hide : fields.hide, // Сохраняем текущее значение, если fields.hide undefined
                    ...fields,
                }
            }
            return col
