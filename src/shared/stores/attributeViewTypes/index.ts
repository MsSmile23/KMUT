...
export interface IAttributeViewTypesStore {
    store: {
        data: IAttributeViewType[]
        state: StoreStates
        error: string
    }
    params: {
    ...
    forceUpdate: () => void
    fetchData: () => void
    setData: (value: IAttributeViewTypesStore['store']['data']) => void
    setState: (value: IAttributeViewTypesStore['store']['state']) => void
    setError: (error?: string) => void
    setStartApiUpdateLoop: (time?: number) => void
    setStopApiUpdateLoop: () => void
    setInitialStoreState: () => void
    getStateStereotypeById: (id: number) => IAttributeViewType
}
export const useAttributeViewTypesStore = create<IAttributeViewTypesStore>()(
    devtools(
        immer((set, get) => ({
            store: {
                data: [],
            ...
            localeName: 'Типы отображения атрибутов',
            setState: (value) => {
                set((state) => {
            ...
            fetchData: async () => {
                ...
                try {
                    const response = await getAttributesViewTypes({ all: true })
...
export const selectAttributeViewTypes = (state: IAttributeViewTypesStore) => state.store.data
export const selectAttributeViewType = (state: IAttributeViewTypesStore) => state.getStateStereotypeById
export const selectAttributeViewTypesUpdateParams = (state: IAttributeViewTypesStore) => {
    return {
        store: state.store,
