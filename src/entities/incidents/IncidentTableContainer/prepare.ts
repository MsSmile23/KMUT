...
export const incidentColumnsKeysTitles = {
    ...
    last_sync_date: 'Дата и время последней попытки синхронизации',
...
export const incidentColumns = Object.entries(incidentColumnsKeysTitles).map(([ key, title ]) => {
    const col = { key, title, dataIndex: key, serverFilterValueKey: key === 'id' ? 'id' : undefined }
    
    if (key == 'id') {
        return ({ ...col, serverFilterValueKey: 'id', defaultSortOrder: 'descend' })
    }
